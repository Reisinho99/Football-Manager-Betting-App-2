...
export interface IStorage {
  ...
  updateMatchStatus(id: number, status: string, homeScore?: number, awayScore?: number): Promise<Match | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async updateMatchStatus(id: number, status: string, homeScore?: number, awayScore?: number): Promise<Match | undefined> {
    const match = await this.getMatch(id);
    if (!match) return undefined;
  ...
  async resolveBet(id: number, status: "WON" | "LOST") {
  ...
  private async initializeDefaultData() {
    ...
    const usTeams = [
      ...
      { name: "Inter Miami", shortName: "MIA", country: "United States", league: "MLS" },
